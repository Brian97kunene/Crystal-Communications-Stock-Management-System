...
const BuildQuery = (dat,data) => {
    ...
    if (dat == "insert") {
         query1 = `INSERT INTO vendor(name,contact)
                    VALUES ('${data}','10112') RETURNING *;
    `;
    }
    else if (dat == "update") {

       
        query1 = `UPDATE vendor SET name = 'MTBTTF' where name='${data}' RETURNING *;`;
    }
    return query1;
...
const SyncTransaction = async  (query, query1,client) => {
    // Express.js Backend Route
        ...
        try {
           ...
           await client.query(BuildQuery("insert",query));
            ...
            await client.query('SAVEPOINT firstpoint'); // 5. Undo everything on error
        
            await client.query(BuildQuery("update",query1)            );
...
async function  CommitTransaction(client) {
    ...
    await client.query('rollback to firstpoint'); // 4. Save changes
...
app.post('/api/purchase', async (req, res) => {
    ...
    try {
        await SyncTransaction(query, query1, client);
        ...
        res.status(200).send(`Transaction failed hard!:`);
    }
...
app.post('/api/bulk-insert', async (req, res) => {
    try {
        ...
        const supp_code = req.body.supply.id;  
        ...
        for (const row in data.rows) {
         ...
         on conflict (sku)
            ...
            updated_on = Now() RETURNING*;`,
                [
                    data.rows[row].name,         //1
                    data.rows[row].description,  //2
                    data.rows[row].sku,          //3
                    data.rows[row].price,        //4
                    data.rows[row].delivery_cost, //5
                    data.rows[row].mark_up,          //6
                    data.rows[row].quantity, // 7
                    '',                     //8
                    '',                      //9
                    '',                     //10
                    data.supply.id //11
...
app.post('/createvendor', async (req, res) => {
    try {
        ...
        console.log("Received data for new vendor:", { name, contact, contact_name, email, address });
        const result = await pool.query(
            'INSERT INTO vendor (name,contact,contact_name,email,address,created_on)  VALUES ($1,$2,$3,$4,$5,now()) RETURNING *', [name, contact, contact_name, email, address]
